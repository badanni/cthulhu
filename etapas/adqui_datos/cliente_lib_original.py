...
"""\package cliente_lib
   ...
   \bug       myTemperature tiene falla por el tipo de dato en formato a python '\x81' toca pasar a '0x81'
...
class cliente_lib:
	...
	def valores_sonares(self,packet):
		...
		dato=range(int(cantidad))
		for j in range(int(cantidad)):
			dato[j]=c #ya que pasa el buffer a la variable esta debe tener la longitud necesaria y deben ser guardada en cada lectura
		for i in range(int(cantidad)):
			packet.bufToStr(dato[i],50)
			dato[i]=dato[i].replace("\x00"," ") #Porque al final anade un NONE del string
			dato[i]=dato[i].strip() 
		self.valor_sonares=[cantidad,dato]
	...
	def devuelve_valors(self):
		...
		try:
			...
			for a in range(len(x_br)):
				self.x.update({'x%d' % self.acu:float(x_br[a])}) #Ordena los datos en forma de diccionario
				self.y.update({'y%d' % self.acu:float(y_br[a])})
				self.t.update({'t%d' % self.acu:0})
				#self.t.update({'t%d' % self.acu:float(t_br[a])})
				self.acu+=1
				if self.acu==30:
					self.acu=0
			#print self.x
			...
			return {'x0':-9999,'x1':-9999},{'y0':-9999,'y1':-9999},{'t0':0,'t1':0}
