...
from AriaPy import *
#from AriaPy import ArMutex # Probar para ver que pasa
...
class cliente_lib:
	...
	def envio_griper(self,client,TransRatio):
		"""
		\brief Sirve para realizar la teleoperacion, mandando los parametros
		\param self este parametro no es necesario escribir
		\param client Se debe trar el objeto cliente a la definicion para poder utilizar el enlace del cliente para enviar el paquete al servidor
		...
		packet=ArNetPacket()
		packet.doubleToBuf(myTransRatio)
		client.requestOnce("ComandoGripper", packet)
		return client
	...
	def cliente_inicio(self):
		...
		#comando de prueba
		client.setConnectTimeoutTime(1)
		print client.getConnectTimeoutTime()
		#
		client.runAsync()
...
def main():
	...
	a=cliente_lib()
	#a.ip="192.168.10.5" #Si el servidor esta en otra maquina 
